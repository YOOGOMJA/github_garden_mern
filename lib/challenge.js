    ...
    if (body) {
                ...
                if (body.start_dt && body.finish_dt) {
                    ...
                    if (update_start_dt._isValid && update_finish_dt._isValid) {
                        ...
                        if (update_start_dt.diff(update_finish_dt) < 0) {
                            // 일자 1번 조건 통과
                            _result.validated.push({ name : "start_dt" , value : update_start_dt.toDate() });
                            _result.validated.push({ name : "finish_dt" , value : update_finish_dt.toDate() });
                        } else {
        ...
        if (_result.has.title) {
            ...
            if (body.title.trim() !== "" && body.title.length <= 30) {
                // 제목 항목 통과
                _result.validated.push({ name : "title" , value : body.title.trim() });
            } else {
                _result.result = false;
                _result.error.push({
                    target: "title",
