    ...
    user_ids.forEach(user_id=>{
        ...
        dates.forEach(date=>{
            ...
            if(mDate.isBefore(now)){
                if(raw[user_id][date] > 0){ result[user_id].attend++; }
                else{ result[user_id].notAttend++; }
            }
        });

        result[user_id].rate = (result[user_id].attend / (result[user_id].attend + result[user_id].notAttend)) * 100
    }); 
