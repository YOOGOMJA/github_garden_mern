...
router.get("/challenge/:challenge_id/request" , async(req, res)=>{
    try{
        ...
        if(!current_challenge){ throw new Error("존재하지 않는 도전기간입니다"); }
        
        const allRequests = 
        await Models.JoinRequest.aggregate([
...
router.put("/challenge/:challenge_id/user/:login/request" , async(req, res)=>{
    try{
        ...
        if(!current_user){ throw new Error("존재하지 않는 사용자입니다"); }
...
// 특정 사용자 반려 처리
router.delete("/challenge/:challenge_id/user/:login/request", async(req, res)=>{
    try{
        if(!req.isAuthenticated()) { throw new Error("로그인이 필요합니다"); }
        if(!req.user.is_admin) { throw new Error("관리자 권한이 필요합니다"); }
        const current_challenge = await Models.Challenge.findOne({
...
router.put("/token/:token" , async(req, res)=>{
    try{
        if(!req.isAuthenticated()) { throw new Error("로그인이 필요합니다"); }
        if(req.user.is_admin){ throw new Error("이미 관리자 계정으로 등록된 계정입니다"); }
        const token = req.params.token;

        const fetchedToken = await Models.AuthToken.findOne({
            value : token,
        });
        if(!fetchedToken){ throw new Error(`[${req.params.token}]는 존재하지 않는 토큰입니다`); }
        const mNow = moment();
        const mExpired = moment(fetchedToken.expired_at);
        if(mNow.diff(mExpired) > 0){ throw new Error("만료된 토큰 입니다");}
        if(fetchedToken.used_by !== null){ throw new Error("이미 사용된 토큰입니다"); }
